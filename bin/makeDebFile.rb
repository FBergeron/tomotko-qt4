...
File.open( 'etc/deb/control' ) {
    ...
    File.open( 'debian/DEBIAN/control', 'w' ) {
        ...
        input.each_line {
            ...
            modifLine = line.sub( /[$]VERSION/, "#{$VERSION}-#{$DVERSION}" )
...
FileUtils.mv( 'debian.deb', "tomotko-#{$VERSION}-#{$DVERSION}_i386.deb" )

# Execute the lintian check
system( 'lintian ' + "tomotko-#{$VERSION}-#{$DVERSION}_i386.deb" )
